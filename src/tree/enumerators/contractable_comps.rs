...
use crate::{tree::{TreeCaseInstance, TreeContext, ContractableCompInstance, utils::get_merge_graph}, proof_logic::{Enumerator, EnumeratorTactic}};
...
impl<'a> Enumerator<ContractableCompInstance, TreeContext> for ContractableCompsEnumerator<'a> {
    fn iter(&self, _context: &TreeContext) -> Box<dyn Iterator<Item = ContractableCompInstance> + '_> {
        let num_comps = self.instance.comps.len();
        let graph = get_merge_graph(&self.instance.comps, &self.instance.edges);

        let iter = self.instance.comps.iter().enumerate().take(num_comps - 1).filter(|(_, c)| c.is_c6()).flat_map(move |(idx, comp)| {
...
impl EnumeratorTactic<TreeCaseInstance, ContractableCompInstance, TreeContext> for ContractableCompsEnum {
    ...
    fn item_msg(&self, item: &ContractableCompInstance) -> String {
        format!("Component {} contractible! Free nodes [{}]", item.contr_idx, item.free_nodes.iter().join(","))
    }

    fn precondition(&self, data: &TreeCaseInstance, _context: &TreeContext) -> bool {
        data.comps.iter().take(data.comps.len() - 1).any(|c| c.is_c6())
    }
