...
fn prove_nice_path<C: CreditInvariant>(path: NicePath, credit_inv: C) -> bool {
    ...
    for f_perm in f_nodes
        ...
        .dedup()
    {
        let (&f_out, &f_in) = (f_perm[0], f_perm[1]);
        for p_perm in p_nodes
            .iter()
            .combinations_with_replacement(2)
            .permutations(2)
            .flatten()
            .dedup()
        {
            let (&p_out, &p_in) = (p_perm[0], p_perm[1]);
            for l_perm in l_nodes
                .iter()
                .combinations_with_replacement(2)
                .permutations(2)
                .flatten()
                .dedup()
            {
                let (&l_out, &l_in) = (l_perm[0], l_perm[1]);
                let cycle = vec![
