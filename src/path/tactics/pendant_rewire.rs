...
use crate::{path::{SelectedMatchingInstance, proof::{Tactic, Statistics}, Matching3, MatchingEdge, PathHit, SuperNode, PathMatchingInstance}, proof_tree::ProofNode};
...
impl Tactic<SelectedMatchingInstance> for PendantRewireTactic {
    ...
    fn action(&mut self, data: &SelectedMatchingInstance, _context: &mut crate::path::proof::ProofContext) -> crate::proof_tree::ProofNode {
        self.num_calls += 1;

        let matching3 = &data.path_matching.matching;
        let other_matched = data.matched.iter().filter(|e| e.1 != matching3.path_edge_left.unwrap().source()).collect_vec();
        ...
        if prelast.is_c6() || prelast.is_large() || prelast.is_complex() {
            ...
            if last.is_c3() {
                ...
                let new_matching = Matching3 {
                    ...
                    path_edge_left: Some(MatchingEdge(new_last_matching_edge.1, PathHit::Path(matching3.source_comp_idx - 1))),
                    path_edge_right: None,
                    other_edges: vec![MatchingEdge(other_matched.last().unwrap().1, PathHit::Path(matching3.source_comp_idx - 1)), MatchingEdge(matching3.path_edge_left.unwrap().source(), PathHit::Path(matching3.source_comp_idx - 2))],
                };
